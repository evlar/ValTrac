...
#def run_nominator_payout():
 #   python_interpreter = sys.executable
  #  script_dir = os.path.dirname(os.path.abspath(__file__))
   # payout_script_path = os.path.join(script_dir, 'blockchain', 'pay.py')
#
    # Ask if the user is running a local or remote Subtensor network
#    subtensor_network_choice = input("Connect to local Subtensor network for payout? (yes/no): ").strip().lower()
 #   if subtensor_network_choice == 'yes':
...
def main_menu():
    while True:
        ...
        print("2. Payout by Referral Structure")
        print("3. Full Payout by Nominator")
        print("4. Add users")
        print("5. Edit users")
        print("6. Add referral")
        print("7. Edit referrals")
        print("8. Charts and Statistics")
        print("9. Exit")
        ...
        if choice == '1':
            ...
            os.system('python3 -m src.blockchain.nominator_payout')
        elif choice == '4':
            add_users_submenu()
        elif choice == '5':
